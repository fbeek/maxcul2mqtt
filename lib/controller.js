...
class Controller {
    ...
    async start() {
        ...
        this.availabilityTimer = setTimeout(this.sendAvailabilityAndLastState.bind(this),1000*60*15);
    }
    ...
    async sendAvailabilityAndLastState(){
        ...
        for(let deviceId in devices){
            ...
            await this.mqtt.publish(deviceId+"/availability", 'online', {retain: true, qos: 0});
            let state = this.db.getDeviceStateById(deviceId);
            await this.mqtt.publish(deviceId+"/state",JSON.stringify(state),{retain: true, qos: 0});
        }
    }
