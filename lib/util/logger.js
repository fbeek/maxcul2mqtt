...
const output = settings.logger.log_output;

// Directory to log to
const timestamp = moment(Date.now()).format('YYYY-MM-DD.HH-mm-ss');
const directory = settings.logger.log_directory.replace('%TIMESTAMP%', timestamp);
const logFilename = settings.logger.log_file.replace('%TIMESTAMP%', timestamp);
...
const level = settings.logger.log_level;
...
const timestampFormat = () => moment().format(settings.logger.timestamp_format);
    ...
    new winston.transports.Console({
        ...
        format: winston.format.combine(
            ...
            winston.format.printf(/* istanbul ignore next */(info) => {
                ...
                const prefix = colorizer.colorize(level, `Maxcul2MQTT:${levelWithCompensatedLength[level]}`);
...
if (settings.logger.log_rotation) {
...
function cleanup() {
    if (settings.logger.log_directory.includes('%TIMESTAMP%')) {
