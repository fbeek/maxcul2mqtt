...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        ...
        const options = {
            will: {
                topic: `${settings.mqtt.base_topic}/bridge/state`,
                payload: 'offline',
                retain: true,
                protocolVersion: 3.1
            },
        ...
        if (mqttSettings.user && mqttSettings.password && mqttSettings.user != "" && mqttSettings.password != "") {
            options.username = mqttSettings.user;
            options.password = mqttSettings.password;
        }

        if (mqttSettings.client_id && mqttSettings.client_id != "") {
    ...
    async publish(topic, payload, options, base=settings.mqtt.base_topic, skipLog=false, ) {
        topic = `${base}/${topic}`;
