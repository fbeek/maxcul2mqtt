...
class MaxculDriver extends EventEmitter {
    ...
    handleIncommingMessage(message) {
        ...
        if (packet) {
            ...
            }else if(packet.forMe !== true && packet.getDest() !== "000000"){
                return logger.debug("ignored, paket is not for us");
            }else {
    ...
    sendPair(dest, pairId, pairType, deviceType) {
        ...
        const masterType = this.getDeviceId(deviceType);
        const payload = Sprintf('%s%02x',pairId,type);
        return this.sendMsg("20",this.baseAddress,dest,payload,"00","00","0"+masterType);
    }
    ...
    WallThermostatState(packet) {
        ...
        this.emit('WallThermostatStateRecieved',WallthermostatState);
    }
